...
def connect(page_id, token):
    '''Connect to FB Graph API and scrape data regarding events for each society.'''
    ...
    writer.writerow(["Society", "event_name", "attendance_count", "interested_count", "no_reply_count", "declined_count", "maybe_count", "total_invited_count", "description", "start_time", "end_time", "place", "event_type" ])
    ...
    for i in range(0, len(data['events']['data'])):
        try:
            ...
            writer.writerow(["180DC", name, attendance_count, interested_count, no_reply_count, declined_count, maybe_count, total_invited_count, description, start_time, end_time, place, type_of_event])
